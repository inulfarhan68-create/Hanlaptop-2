...
export function CrmManagement() {
  ...
  const { data: pointsData, mutate: mutatePoints, isLoading: pointsLoading } = useSWR<any[]>(
    (import.meta.env.VITE_API_URL || '') + '/api/crm/points'
  )
  const { data: remindersData, mutate: mutateReminders, isLoading: remindersLoading } = useSWR<any[]>(
    (import.meta.env.VITE_API_URL || '') + '/api/crm/reminders'
  )
  ...
  const handleSendReminder = async (reminder: any) => {
    ...
    try {
      ...
      const res = await fetch((import.meta.env.VITE_API_URL || '') + `/api/crm/reminders/${reminder.id}/send`, {
        method: 'POST',
      })
  ...
  const handleUpdateWarrantyStatus = async (claimId: string, newStatus: string) => {
    ...
    try {
      const res = await fetch((import.meta.env.VITE_API_URL || '') + `/api/crm/warranty/${claimId}/status`, {
        method: 'PUT',
