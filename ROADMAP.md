...
| --- | --- | --- |
| High ✔️ diperbaiki | **3 kebocoran lintas-tenant** (ditemukan lewat audit 121 file route): (a) `GET /api/user/stores` mengembalikan **seluruh** store di database untuk peran owner — padahal pasca-Fase-2 owner itu org-scoped — sehingga store switcher membocorkan nama & alamat tenant lain; (b) fallback identitas toko berisi **kontak asli Han Laptop** dan tercetak di nota tenant lain (lihat Technical Debt #11); (c) `GET /api/suggestions` ber-auth tapi tanpa `storeScope`, sehingga autocomplete menyodorkan nama barang & perangkat **semua tenant**. Semua ditutup + dijaga e2e. | `api/user/stores`, `api/settings`, `api/suggestions` |
...
| Medium | Pagination masih kurang di beberapa list endpoint. **Piutang/hutang sudah beres** (`api/receivables`/`api/payables`, agregat di SQL + paging); `inventory` sudah punya. Sisanya (`customers`, `suppliers`, `services`, `logs`) belum — dan `transactions` **tak bisa** dibatasi begitu saja karena beberapa konsumen masih menjumlahkan sisi klien. | berbagai route |
