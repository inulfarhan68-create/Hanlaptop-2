...
**SaaS / billing / demo:** `BILLING_WEBHOOK_SECRET` (gate `api/webhooks/billing`), `BILLING_CONTACT_WHATSAPP` + `BILLING_CONTACT_EMAIL` + `BILLING_BANK_INFO` (kanal perpanjangan manual yang ditampilkan di `/settings/billing` — pembayaran belum lewat gateway; **kalau kosong barisnya dihilangkan**, jangan diisi contoh/placeholder, lihat aturan no. 15), `DEFAULT_ORGANIZATION_ID` (flagship tenant id for `migrate-tenancy`, fallback `org-default`), `PLATFORM_ADMIN_EMAIL` (designate the platform operator in `migrate-tenancy`), `DEMO_LOGIN_EMAIL` + `DEMO_LOGIN_PASSWORD` (read-only demo account for `api/demo/login` — must match `seed-demo-tenant`), `DEMO_ORGANIZATION_ID`/`DEMO_STORE_ID` (demo tenant ids, fallback `org-demo`/`store-demo`).
...
18. **Ubah matriks fitur → wajib `npm run db:sync-plans`.** `requireFeature()` membaca kolom `features` di **baris tabel `plans`**, bukan konstanta TypeScript-nya. Jadi menambah key di `FEATURES`/`PLAN_SEED` tak berpengaruh apa pun sampai plans di-seed ulang — dan gagalnya **senyap serta terbalik**: paket yang baru diberi fitur justru ditolak, dan tenant `internal` (yang mestinya dapat semua) kehilangan fitur yang baru ditambahkan. Persis itu yang terjadi saat gating AI dipasang: tenant demo di paket internal dibalas *"your current plan does not support the 'aiPricing' feature"*. Jalankan sync **sebelum** deploy kodenya (urutan ini aman: tanpa gate, plans yang sudah diperbarui tak mengubah apa-apa; sebaliknya kode duluan = fitur mati sampai sync).
