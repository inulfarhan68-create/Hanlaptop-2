...
export function Sidebar({ user }: { user?: any }) {
  ...
  // Fetch alerts
  const { data: alertsData } = useSWR((process.env.NEXT_PUBLIC_API_URL || '') + '/api/alerts', {
  ...
  const { data: approvalsData } = useSWR(
    isOwner || user?.role === "manager" 
      ? (process.env.NEXT_PUBLIC_API_URL || '') + '/api/approvals?status=PENDING' 
      : null,
  ...
  const { data: activeShiftData, mutate: mutateShift } = useSWR(
    selectedStoreId !== 'all' ? (process.env.NEXT_PUBLIC_API_URL || '') + '/api/shifts/active' : null
  )
  const activeShift = activeShiftData?.activeShift || null
  const { data: storeSettings } = useSWR<any>((process.env.NEXT_PUBLIC_API_URL || '') + '/api/settings')

  const cachedEnableShift = typeof window !== "undefined" ? localStorage.getItem("enableCashierShift") : null
