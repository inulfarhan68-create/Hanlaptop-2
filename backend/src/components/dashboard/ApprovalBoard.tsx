...
export function ApprovalBoard({ isOwner }: { isOwner: boolean }) {
    ...
    const handleAction = async (requestId: string, action: 'APPROVE' | 'REJECT') => {
        ...
        fetchRequests(updatedRequests, false); // mutate locally first
        ...
        try {
            ...
            fetchRequests(); // rollback on failure
        } finally {
    ...
    if (loading) return <div className="p-4 text-sm text-muted-foreground animate-pulse border rounded-xl mb-4 h-[72px] bg-card flex items-center justify-center">Memuat persetujuan...</div>;
