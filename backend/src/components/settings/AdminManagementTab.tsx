...
export function AdminManagementTab() {
  ...
  const { data: branchesList } = useSWR('/api/stores')
  ...
  const handleSaveEditUser = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const resUser = await apiFetch(`/api/users/${editingUser.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
      ...
      const resAssign = await apiFetch(`/api/users/${editingUser.id}/assign-store`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
