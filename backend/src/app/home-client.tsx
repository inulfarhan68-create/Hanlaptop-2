...
export default function LandingPage() {
  ...
  const [selectedBranch, setSelectedBranch] = useState<string>(() => typeof window !== "undefined" ? localStorage.getItem("selectedBranchSlug") || "default" : "default")
  ...
  useEffect(() => {
    const fetchStoreInfo = async () => {
      try {
        const res = await apiFetch(`/api/public/catalog/${selectedBranch}`)
        if (res.ok) {
  ...
  const submitJualLead = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await apiFetch(`/api/public/buyback`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  const submitServisBooking = async (e: React.FormEvent) => {
    ...
    try {
      const res = await apiFetch(`/api/public/service/booking`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          storeId: storeInfo?.store?.id || "default",
  ...
  const handleTrackStatus = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const resService = await apiFetch(`/api/public/service/${trackInvoice}`)
      if (resService.ok) {
      ...
      const resInvoice = await apiFetch(`/api/public/invoice/${trackInvoice}`)
      if (resInvoice.ok) {
      ...
      <nav className="sticky top-0 z-50 bg-white/90 backdrop-blur-md border-b border-slate-100 px-4 lg:px-8 py-4 transition-all duration-300">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-3">
            ...
            <Link href={`/catalog/${selectedBranch}`}>
        ...
        {mobileMenuOpen && (
          <div className="md:hidden mt-4 pt-4 border-t border-slate-100 flex flex-col gap-3 font-semibold text-slate-500 text-sm animate-in slide-in-from-top duration-200">
            ...
            <div className="flex flex-col gap-2 pt-2">
              <Link href={`/catalog/${selectedBranch}`} onClick={() => setMobileMenuOpen(false)}>
                    ...
                    >
                      ...
                      <div className="p-4 pt-0">
                        <div className="pt-3 border-t border-slate-100 flex items-center justify-between">
                          ...
                          <Link href={`/catalog/${selectedBranch}?item=${item.id}`}>
        ...
        <div className="max-w-6xl mx-auto grid grid-cols-1 md:grid-cols-4 gap-8">
          ...
          <div className="space-y-3">
            ...
            <ul className="space-y-1.5 text-xs text-slate-550">
              <li><Link href={`/catalog/${selectedBranch}`} className="hover:text-slate-900 cursor-pointer transition-colors">Katalog Laptop</Link></li>
              <li><button onClick={() => {
                setActiveSection("jual")
                setJualSubmitted(false)
                setJualResult(null)
