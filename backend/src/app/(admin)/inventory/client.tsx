...
import { TrendingDown, Info, Image as ImageIcon, Upload, Sparkles, Lock } from "lucide-react"
import { useFeatureGate } from "@/components/PlanFeaturesProvider"
...
export function InventoryClient({ user }: { user: any }) {
  ...
  const currentPageParams = parseInt(searchParams.get("page") || "1", 10)

  // Local state for INSTANT client-side filtering without Next.js dev server lag
  ...
  const [localPage, setLocalPage] = useState(currentPageParams)

  // Sync with URL on mount or back/forward
  useEffect(() => {
  ...
  const setCurrentPage = (val: number | ((prev: number) => number)) => {
    const nextVal = typeof val === 'function' ? val(localPage) : val
    setLocalPage(nextVal)
    updateParams({ page: nextVal.toString() })
  }
  const setFilterCategory = (val: string) => {
    setLocalCategory(val)
    setLocalPage(1)
    updateParams({ category: val !== "all" ? val : null, page: "1" })
  }
  const setFilterStatus = (val: string) => {
    setLocalStatus(val)
    setLocalPage(1)
    updateParams({ status: val !== "all" ? val : null, page: "1" })
  }
  ...
  const inventorySWRKey = ["inventory", "", localPage, itemsPerPage, localCategory, localStatus, deferredSearchTerm]
  const { data: inventoryData, error: inventoryError, isLoading: loading, mutate: mutateInventory } = useSWR(
    inventorySWRKey,
    async () => {
      const res = await apiFetch(`/api/inventory?${queryParams}`)
      return res.json()
    },
  ...
  const { data: techniciansData } = useSWR<any[]>('/api/technicians')
  const { data: suppliersData } = useSWR<any[]>('/api/suppliers')
  ...
  const { data: itemPassports, isLoading: passportsLoading } = useSWR<any[]>(
    viewDetailItem?.id ? `/api/inventory/passports?inventoryId=${viewDetailItem.id}` : null
  )
  ...
  // Toolbar actions that belong to a paid tier. Their APIs already answer 402,
  // so without this the button opened a modal that failed on submit.
      ...
      <div className="shrink-0 flex flex-col gap-1.5 md:gap-2 bg-white/80 light-blue:bg-white dark:bg-card backdrop-blur-xl rounded-xl md:rounded-[2rem] border border-border shadow-sm z-40 relative p-2 md:px-5 md:py-3 mt-0 mb-2">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button size="sm" variant="outline" className={`flex items-center gap-1.5 h-8 px-3 text-[11px] font-bold hover:bg-rose-500/10 hover:text-rose-600 border-slate-200 dark:border-slate-800 cursor-pointer ${markdownGate.allowed ? "" : "opacity-60"}`} onClick={() => markdownGate.allowed ? setIsMarkdownOpen(true) : markdownGate.notifyLocked()} title="Markdown Liquidator (Stok Mati)">
              <TrendingDown className="h-3.5 w-3.5 text-rose-500" /> <span className="hidden md:inline">Markdown</span>
              {!markdownGate.allowed && <Lock className="h-3 w-3 opacity-70" />}
            </Button>
            <Button size="sm" variant="outline" className={`flex items-center gap-1.5 h-8 px-3 text-[11px] font-bold hover:bg-indigo-500/10 hover:text-indigo-600 border-slate-200 dark:border-slate-800 cursor-pointer ${flyerGate.allowed ? "" : "opacity-60"}`} onClick={() => flyerGate.allowed ? setIsFlyerOpen(true) : flyerGate.notifyLocked()} title="Buat Poster Flyer Update Stok">
            ...
            {canWrite && localStorage.getItem('selectedStoreId') !== 'all' && (
              <>
                <Button size="sm" variant="outline" className={`flex items-center gap-1.5 h-8 px-3 text-[11px] font-bold border-slate-200 dark:border-slate-800 cursor-pointer hover:bg-slate-100 dark:hover:bg-slate-800 ${importGate.allowed ? "" : "opacity-60"}`} onClick={() => importGate.allowed ? setIsImportOpen(true) : importGate.notifyLocked()} title="Impor data barang dari Excel atau Nota PDF AI">
                  ...
                  {!importGate.allowed && <Lock className="h-3 w-3 opacity-70" />}
                </Button>
      ...
      {!loading && items.length > 0 && (
        <div className="flex items-center justify-between px-2 py-2 shrink-0 bg-card rounded-xl border mt-2">
          <p className="text-[10px] md:text-xs text-muted-foreground">
            Menampilkan <span className="font-medium text-foreground">{(localPage - 1) * itemsPerPage + 1}</span>-
            <span className="font-medium text-foreground">{Math.min(localPage * itemsPerPage, meta.totalItems)}</span> dari <span className="font-medium text-foreground">{meta.totalItems}</span> barang
          </p>
          <div className="flex gap-1">
            ...
            <Button
              ...
              disabled={localPage === Math.ceil(meta.totalItems / itemsPerPage) || meta.totalItems === 0}
            >
              Selanjutnya
            </Button>
          </div>
                        ...
                        return (
                          <span 
                            ...
                            onClick={() => {
                              ...
                              router.push(`/passports?sn=${encodeURIComponent(passport.serialNumber)}`);
                            }}
                            className={cn(
