...
export default function DigitalPassportClient() {
  ...
  const { data: passport, error: passportError, mutate: mutatePassport } = useSWR(
    activeSn ? `/api/inventory/passports/by-sn/${encodeURIComponent(activeSn)}` : null
  );

  // Fetch warranty data
  const { data: warrantyData } = useSWR(
    activeSn ? `/api/warranty/check?sn=${encodeURIComponent(activeSn)}` : null
  );

  // Fetch claims
  const { data: claims, mutate: mutateClaims } = useSWR(
    activeTab === "claims" ? `/api/warranty-claims` : null
  );

  // Fetch lifecycle data (timeline)
  const { data: lifecycleData, mutate: mutateLifecycle } = useSWR(
    passport?.id ? `/api/inventory/passports/${passport.id}/lifecycle` : null
  );

  // Fetch technicians for refurbishment form
  const { data: technicians } = useSWR('/api/technicians');

  // Fetch inventory for autocomplete during seeding
  const { data: inventoryItems } = useSWR(showSeeding ? `/api/inventory` : null);
  ...
  const handleSeed = async (e: React.FormEvent) => {
    ...
    try {
      const res = await apiFetch(`/api/inventory/passports`, {
        method: 'POST',
  ...
  const handleClaimWarranty = (result: any) => {
    // Next has no router state; hand the claim prefill to Services via
    // sessionStorage — never the URL, which would leak customer PII. Services
    // reads and clears "warrantyClaimState" on mount.
    sessionStorage.setItem("warrantyClaimState", JSON.stringify({
      mode: "claim",
    ...
    router.push("/services?mode=claim")
  };

  const handleResolveClaim = async (claimId: string) => {
    try {
      const res = await apiFetch(`/api/warranty-claims/${claimId}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          status: 'COMPLETED',
  ...
  const handleSaveHardwareId = async (data: any) => {
    try {
      const res = await apiFetch(`/api/inventory/passports/${passport.id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
