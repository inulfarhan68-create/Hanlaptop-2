...
import type { PublicServiceData } from "@/lib/public/services"

export default function NotaServisClient({ data: initialData, id }: { data: PublicServiceData; id: string }) {
  ...
  const handleSubmitRating = async (e: React.FormEvent) => {
    ...
    try {
      const res = await apiFetch(`/api/public/service/${id}`, {
        method: 'PATCH',
            ...
            <div className="flex items-start gap-6">
              <div className="w-32 h-auto flex items-start justify-center shrink-0">
                {storeSettings.storeLogo ? (
                  ...
                  <img src={assetUrl("/logo-print.png")} alt="Logo" className="w-full h-auto object-contain" onError={(e) => e.currentTarget.style.display = 'none'} />
                )}
              </div>
              <div className="flex flex-col pt-2">
                {/* Placeholder address/phone must never print on a real receipt — a
