...
import type { PublicCatalogData } from "@/lib/public/catalog"
...
export default function CatalogClient({ initialData, slug }: { initialData: PublicCatalogData; slug: string }) {
  ...
  const setSearchParams = (updater: (prev: URLSearchParams) => URLSearchParams, options?: { replace?: boolean }) => {
    const next = updater(new URLSearchParams(nextSearchParams?.toString() || ''))
  ...
  const [data, setData] = useState<PublicCatalogData | null>(initialData)
  ...
  // Refetch when slug changes (e.g. branch switch)
  const fetchCatalog = async (targetSlug?: string) => {
    try {
      setLoading(true)
      const res = await apiFetch(`/api/public/catalog/${targetSlug || slug}?t=${Date.now()}`, {
        cache: 'no-store'
      })
      if (!res.ok) {
