/**
 * The chart of accounts a new store is seeded with — the single source of
 * truth for every account code in the app.
 *
 * Kept in its own module, free of any database import, so pure code can read it.
 * `account-code-lookup` derives its name→code map from this list, and a test
 * imports it; pulling in seed-coa.ts would have dragged a live connection into
 * both.
 */
